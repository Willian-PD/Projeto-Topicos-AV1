...
export default class ViewerTrabalhoEscolar {
  ...
  constructor(ctrl) {
    ...
    this.tfMatriculaAluno = this.obterElemento('tfMatriculaAluno');
    this.tfDisciplina = this.obterElemento('tfDisciplina');
    this.tfTema = this.obterElemento('tfTema');
    this.tfNota = this.obterElemento('tfNota');
...
//------------------------------------------------------------------------//
  ...
  apresentar(pos, qtde, trabalhoEscolar) {    
    
    this.configurarNavegacao( pos <= 1 , pos == qtde );   

    if(trabalhoEscolar == null) {
      this.tfMatriculaAluno.value = "";
      this.tfDisciplina.value       = "";
      this.tfTema.value      = "";
      this.tfNota.value      = "";
      this.divAviso.innerHTML = " Número de Trabalhos Escolares: 0";
    } else {
      this.tfMatriculaAluno.value = trabalhoEscolar.getMatriculaAluno();
      this.tfDisciplina.value      = trabalhoEscolar.getDisciplina();
      this.tfTema.value     = trabalhoEscolar.getTema();
      this.tfNota.value      = trabalhoEscolar.getNota();
      this.divAviso.innerHTML = "Posição: " + pos + " | Número de Trabalhos Escolares: " + qtde;
    }
  }
...
//------------------------------------------------------------------------//
  ...
  statusEdicao(operacao) { 
    ...
    if(operacao != Status.EXCLUINDO) {
      this.tfMatriculaAluno.disabled = false;
      this.tfDisciplina.disabled = false;
      this.tfTema.disabled = false;
      this.tfNota.disabled = false;
...
function fnBtOk() {
  const matriculaAluno = this.viewer.tfMatriculaAluno.value;
  const disciplina = this.viewer.tfDisciplina.value;
  const tema = this.viewer.tfTema.value;
  const nota = this.viewer.tfNota.value;
  ...
  this.viewer.getCtrl().efetivar(matriculaAluno, disciplina, tema, nota); 
